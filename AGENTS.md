...
| Analytics | **Custom cookieless on D1** | Zero-dependency `@gkoreli/analytics` package. Cloudflare Workers + D1. Visitor classification (human/bot/AI) via UA regex. Public `/api/stats` endpoint with timezone-aware date grouping (`tz` param), visitor type filter (`visitor` param: human/bot/ai/all), and adaptive granularity (hourly for ≤7d, daily for longer). No cookies, no fingerprinting, no third-party scripts. All date operations in `packages/analytics/src/dates.ts`. See ADR-0004. |
| Stats dashboard | **Public `/stats` page with uPlot** | uPlot (9.9K⭐, 24.6KB gz) for trend chart (hourly or daily), visitor type toggle (Humans/Bots/AI/All), period pills (7d/30d/90d/All). Per-page bundle isolation — `stats.js` + `stats.css` load only on `/stats`. All date operations in `packages/blog/src/lib/dates.ts`. See ADR-0005. |
...
- **Analytics**: Custom cookieless analytics — `@gkoreli/analytics` on Cloudflare Workers + D1 (ADR-0004), public `/stats` dashboard with uPlot (ADR-0005). Date utilities: `packages/analytics/src/dates.ts` (server), `packages/blog/src/lib/dates.ts` (client/build). Observability logs enabled in `wrangler.jsonc`.
