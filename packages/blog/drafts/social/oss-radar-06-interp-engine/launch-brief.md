...
- Visual proof: the preamble animation. The block lights up point by point, the TransformerLens ledger ticks each translation, and the `hook_mlp_out` leader first lands on the raw tap with ≠, then swings to the post-norm tap with ✓.
...
The number that carries the post is 0.87: the last-token cosine between the two tensors on gemma-2-2b layer 4 measured on my Mac, against 1.00000 on gpt2 for the same pairing.
