...
The material served change occurs on September 8 in the author's timezone, matching the existing `lastModified` date. The original publication date remains September 6. The `shape-article` validator and production build passed; the build generated all 25 posts and passed the referral-policy integrity check. [Seven generated-output checks](13-public-transparency-checks.json) passed, including all 21 exact prompts, the shorter D1 treatment, explicit residual abuse paths, and unchanged frozen accounting. An independent read-only editorial review found no remaining material issue after clarifying that the D1 measurement counts row reads rather than stored rows. No production SQL or suspicious-destination visit was performed.

Released in `e0e1fa2`; Cloudflare build `ee5b909d-b533-405f-9593-7c6922e767af` completed successfully at September 9 **02:06:10 UTC**. [Live acceptance](14-public-transparency-live.json) at **02:10:21 UTC** verified the public-risk argument, exact generated article content and structured metadata, all 21 rendered prompts, the explicit frozen scope, and the revised content on the Markdown endpoint. The citation endpoint retains the article title. These are HTTP/content checks, not interactive browser QA. TASK-0139's release acceptance is complete.

The owner's subsequent question about deterministic defenses is preserved in a [separate research record](../referral-defense-guarantees/00-evidence-and-options.md), committed in `39df6c4`. That study identifies enforceable guarantees and prior-art limits without expanding this article's body or frozen artifact set.

## Referrers remain useful evidence

September 9 UTC / September 8 PDT, 2026. [TASK-0143](../../../../../docs/tasks/TASK-0143-calibrate-article-025-explanation-of-referrer-evidence.md) records the owner's correction to the opening bullet. The previous wording emphasized the absence of conclusive proof so strongly that it obscured the normal value of referrer data. This was an editorial overcorrection: incomplete or forgeable evidence can still support useful inferences. The title-like instruction to avoid inventing an audience made the dismissal sound especially categorical.

The revised bullet states the value before the limit: referrers help identify traffic sources, while forged or missing values can distort the report. The background now explains ordinary browser referral behavior before the abuse mechanism. The opening, incident interpretation, and ending distinguish the specific suspicious request pattern from the usefulness of referrers generally. The existing fraud limits, exclusions, and historical counts remain. This changes the explanation, not the referral policy or collection.

[MDN's Referer reference](https://developer.mozilla.org/en-US/docs/Web/HTTP/Reference/Headers/Referer), checked September 9 UTC, explains identifying referring pages and using the field for analytics. [RFC 9110 §10.1.3](https://www.rfc-editor.org/rfc/rfc9110.html#section-10.1.3) defines the field and its omission/truncation conditions. These support normal utility and incomplete coverage; the existing Matomo evidence supports the forgery mechanism. None supplies a measured accuracy percentage for this blog.

Editorial lesson: explain what a signal helps us infer before explaining what it cannot establish. Require case-specific evidence to move from a possible failure mode to suspicion of a particular cluster. Preserve this correction here without creating another general skill rule from one article.
...
Released in `b4e14b5`; Cloudflare build `eb957206-a149-40d8-92b7-726c33b9c034` completed successfully at September 9 **02:39:36 UTC**. [Live acceptance](17-practices-live.json) at **02:41:14 UTC** matched the complete served article body, structured metadata, and Markdown to the isolated build; all 23 complete prompts matched, five tables rendered, and the frozen scope remained explicit. This also accepts TASK-0143's calibrated referrer wording from `bc5bf19`. Both editorial tasks are complete. No new analytics measurement or interactive browser QA is claimed. The isolated checkout contains build copies; the authoritative edits and acceptance evidence are committed on `main`.
