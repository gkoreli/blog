...
| ChatGPT iPhone app on Wi-Fi, `/probe/chatgpt-mobile` | 05:22:17 | "HTTP status: 404 Not Found. Main heading: No heading was returned. The fetch failed at the HTTP layer and exposed no page body or <h1> text, so I won't invent one." | 1: `ChatGPT-User/1.0`, same header set as the web run, AS8075, IP in chatgpt-user.json | mobile app uses the same fetcher |
