...
## Audit and corrective revision, 2026-09-03 07:20 UTC

The article was published (by push at 67e06e2) before the full fact-check pass that AGENTS.md requires and TASK-0107 listed. That is a process exception, recorded here. A Codex read-only audit of the article against every artifact and the worklist ran afterwards (`07-audit-2026-09-03.md`, 4 blocker-class and about 50 should-fix findings). Revision 2 of the article addresses them: exact prompts printed; runs, prompts, requests and URL counts recomputed from `data/captures.jsonl`; absence claims scoped to the pages, files and lists checked; the Codex late arrival and the Perplexity answer labelled as hypotheses where the mechanism is inferred; Grok's undetectability scoped to the anonymous proxy path; signing claims scoped to observed requests and served directories; data boundary described honestly with three files; headings name their subjects; literal wording replaces figurative phrases; `lastModified` set and a dated changelog added.
