...
It shows first-hand that on Apple Silicon the eager backend matches `HookedTransformer` on the residual and contribution points at fp32, that steering arithmetic matches a hand-written hook, and that the naming trap the maintainers describe is large on Gemma-2 and absent on gpt2. Sections 1–3 do not test the vLLM backends (CUDA only), bf16, attention-matrix points, more than one prompt, or prompts longer than 128 tokens. Section 4, run the next morning, is the fp16 four-layer sweep proposed in `03` as a verdict-changing test.

## 4. fp16 all-points sweep (2026-09-02)

The article's first verdict-changing test, run as written: interp-engine eager against same-dtype **plain transformers hooks** (an independent comparator; TransformerLens not involved) on the Apple M5 at float16, eight prompts at each of 64, 512 and 2,048 tokens, every applicable canonical point at layers 0, 13, 19, 25 (gemma-2-2b) and 0, 3, 6, 11 (gpt2 control; 2,048 excluded because the checkpoint has 1,024 positions), 32 greedy tokens per prompt. Harness: `repro/scripts/fp16_sweep.py` via `run_fp16_sweep.sh`; results `repro/results/fp16_sweep.json` and `fp16_sweep_summary.md`; runtime 40 minutes (08:02–08:42 PT).

| Model | Applicable points | Comparisons | Max abs diff (finite points) | Min position cosine | Generations identical | Median tok/s, 512-token prompt (interp-engine / plain) | Peak Metal driver memory |
|---|---:|---:|---:|---:|---:|---:|---:|
| gemma-2-2b | 19 | 1,608 | 0 | 0.99999999 | 24 / 24 | 4.1 / 7.9 | 13.3 GiB / 13.3 GiB |
| gpt2 | 18 | 1,008 | 0 | 0.99999999 | 16 / 16 | 34.9 / 44.5 | 1.6 GiB / 1.6 GiB |

Every finite-valued comparison was bit-identical. The harness's own extra criterion "zero non-finite values" fails on `attn_scores` only: the additive causal and sliding-window mask sets masked positions to negative infinity, and both sides have the same count (8,290 on gemma-2-2b, 339,272 on gpt2) in the same places. That criterion was not part of the article's pass mark; on the article's pass mark (exact shapes, mean cosine ≥ 0.9999, max abs ≤ 0.002, no position below cosine 0.99, identical greedy tokens) the sweep passes for both models. Excluded points: `q_norm_*`, `k_norm_*` (no QK norm on these architectures), `attn_gate`, router and expert points, and `mlp_pre_linear` on gpt2 (ungated MLP), all per interp-engine's own registry. Only `attn_scores` was recomputed on the plain side (from post-RoPE Q/K with the family's scale, GQA expansion, Gemma softcap and mask); every other point was a module hook.

**Loader defect found (Reproduced):** `load_model(device='mps', dtype='float16')` hands transformers `device_map='mps'`; on this machine (torch 2.13.0, transformers 5.14.1, macOS 26.5.2) the streamed bf16→fp16 load of gemma-2-2b segfaults inside `transformers/core_model_loading.py:_materialize_copy` (worker thread) or hangs past six minutes. Plain transformers with `device_map='mps'` shows the same hang; `from_pretrained(dtype=float16)` on CPU followed by `.to('mps')` works in about ten seconds. The fp32 direct-to-MPS path used in sections 1–3 was unaffected. The sweep loads interp-engine's model on CPU and moves `hf_model` to MPS; `EagerModel.device` derives from the parameters, so capture and generation run on MPS. Trace: `repro/results/logs-interp-engine-fp16-mps-load-segfault.txt`. This is worth an upstream report; it is a transformers/torch-MPS interaction that interp-engine's deliberate "hand the destination to from_pretrained" design walks into.

Throughput at fp16 is reported as measured: interp-engine's `generate_stream` decoded at about half the rate of plain `generate` on both models in this run. That is a different decode loop, not a parity failure.
