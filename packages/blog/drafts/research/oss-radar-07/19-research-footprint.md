...
The original article release is separately documented in [artifact 18](18-publication-verification.md). The footprint revision also clarifies that the OpenRouter connector test used local servers and made-up responses; no OpenRouter service or model was called. **Published and verified:** commit [`9055d8a`](https://github.com/gkoreli/blog/commit/9055d8a) contains the frozen manifest, matching metadata, and clarified opening. The isolated 26-post production build and client/Worker type checks passed. The focused accounting regression and strict extractor/models/test type check passed. Rendered checks matched all footprint values and exclusions, all fifteen prompt messages, 38 source cards, and the 12-minute reading time (2,396 narrative words including the title).
