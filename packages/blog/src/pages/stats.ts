...
export function statsPage() {
  ...
  <aside class="stats-methodology" aria-labelledby="stats-methodology-heading">
    ...
    <p>Bots and AI UAs are heuristic classes matched on sender-provided User-Agent rules. Browsers are requests whose User-Agent looks like a browser and that also arrived shaped like a browser navigation: a Sec-Fetch-Mode of navigate, a Sec-Fetch-Dest of document, an Accept header that includes HTML, and an Accept-Language header, from a network that is not on a short list of cloud and VPS providers. Browser-like are browser User-Agents that were checked and failed one of those tests. Rows recorded between August 26 and ${evidenceDate} carry no evidence, because the site did not collect it yet; they stay in Browsers so the series is continuous, and they are known to include automation that the checks would have caught. Rows from the browser-beacon era count as Browsers because they ran JavaScript in a browser by construction. Expect the Browsers series to drop at ${evidenceDate}: that is the method changing, not readers leaving.</p>
